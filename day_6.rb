def all_yes(answers)
  sum = 0
  no = 0
  answers[0].each_char do |a|
    (1).upto(answers.size - 1) do |num|
      if !answers[num].include?(a)
        no += 1
      end
    end
    if no == 0
...
a = ["fegnzr", "rzfgne", "erfnzg", "znfreg"]
p all_yes(a)


p 'wdcmlzfnugqtvjbsahi
easrkmocxbpjgi
...
pvuafthmr
dvpmwcyg'.split(/^\s/).map {|i| i.chomp.split(' ')}.map {|i| all_yes(i)}.reduce(:+)
...
#.split(/^\s/).map {|i| i.chomp.split(' ')}.map {|i| i.reduce(:+)}.map {|i| i.split('')}.map {|i| i.uniq}.map {|i| i.size}.reduce(:+)