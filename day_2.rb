...
def extract_line(line)
  ...
  keyword = split_text[1][0]
  password = split_text[2]
  range = split_text[0].split('-').map(&:to_i)

  [range, keyword, password].flatten
end

def valid_password?(credentials)
  count = credentials[3].count(credentials[2])
  count >= credentials[0] && count <= credentials[1]
end

def valid_password_2?(cred)
  count = 0

  count += 1 if cred[3][cred[0] - 1] == cred[2]
  count += 1 if cred[3][cred[1] - 1] == cred[2]
  count == 1
end
...
1-3 h: zhzzt
2-11 j: sjjrtjkjhjj
6-7 m: mlmrrmm'.each_line do |l|
  valid_count += 1 if valid_password_2?(extract_line(l.chomp))
end
