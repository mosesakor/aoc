...
final = []
p pass_input = 'eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
...
hcl:#888785 cid:108 hgt:160cm'.split(/^\s/).map {|i| i.chomp.split(' ')}.select {|i| i.size > 6}

pass_input.each do |i|
  if i.size == 8
   p x = i.sort[0]
   p y = i.sort[2..-1]
...
def valid_byr(byr)
  byr.size == 4 && byr.to_i >= 1920 && byr.to_i <= 2002
end

def valid_iyr(iyr)
  iyr.size == 4 && iyr.to_i >= 2010 && iyr.to_i <= 2020
end

def valid_eyr(eyr)
  eyr.size == 4 && eyr.to_i >= 2020 && eyr.to_i <= 2030
end

def valid_hgt(hgt)
  unit = hgt[-2..-1]
  if unit == 'cm' && hgt[0..-3].to_i >= 150 && hgt[0..-3].to_i <= 193
    true
  elsif unit == 'in' && hgt[0..-3].to_i >= 59 && hgt[0..-3].to_i <= 76
    true
  else
    false
  end
...
def valid_ecl(ecl)
  ['amb', 'blu', 'gry', 'grn', 'hzl', 'oth', 'brn'].include?(ecl)
end
...
def count_passport(input)
  ...
  input.each do |i|
    ...
    count += 1 if valid_byr(byr) && valid_ecl(ecl) && valid_eyr(eyr) && valid_hcl(hcl) && valid_hgt(hgt) && valid_iyr(iyr) && valid_pid(nid)
  end
  count
end

p count_passport(final)

pass_input = final[-7]

  p byr = valid_byr(pass_input[0].split(':')[1])
  p ecl = valid_ecl(pass_input[1].split(':')[1])
  p eyr = valid_eyr(pass_input[2].split(':')[1])
  p hcl = valid_hcl(pass_input[3].split(':')[1])
  p hgt = valid_hgt(pass_input[4].split(':')[1])
  p iyr = valid_iyr(pass_input[5].split(':')[1])
  p nid = valid_pid(pass_input[6].split(':')[1])

# count_passport(pass_input)

# .split(/^\s/).map {|i| i.chomp.split(' ').map {|i| i[0..2]}}.select {|i| i.sort == fields[0] or i.sort == fields[1]}.size